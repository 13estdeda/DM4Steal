...
def find_label_embed(adj,embed,feature,adj_shadow,embed_shadow,feature_shadow,batch):
    # adj=adj.cpu().detach()
    ...
    csr_adj_shadow=adj_shadow.numpy()
    csr_adj_shadow=sp.csr_matrix(csr_adj_shadow)
    adj_tuple_shadow=sparse_to_tuple(csr_adj_shadow)
    edge_shadow=adj_tuple_shadow[0]
    edge_all_shadow=sparse_to_tuple(csr_adj_shadow)[0]
    all_edge_list_shadow=list(range(edge_shadow.shape[0]))
    np.random.shuffle(all_edge_list_shadow)
    num_train_shadow=int(np.floor(edge_shadow.shape[0]/2))
    train_edge_index_shadow=all_edge_list_shadow[:num_train_shadow]
    train_edge_shadow=edge_shadow[train_edge_index_shadow]
    ...
    train_edge_false=[]
    while len(train_edge_false)<(num_train_shadow):
        idx_i = np.random.randint(0, adj_shadow.shape[0]-2)
        idx_j = np.random.randint(0, adj_shadow.shape[0]-2)
        if idx_i == idx_j:
            continue
        if ismember([idx_i, idx_j], edge_shadow):
            continue
        if ismember([idx_j, idx_i], edge_shadow):
            continue
        if train_edge_false:
    ...
    train_edge_all=np.vstack((train_edge_shadow,train_edge_false))
    ...
    for i in range (len(addr_train)):
        embed_train_0.append( embed_shadow[train_edge_all[i][0]])
        embed_train_1.append(embed_shadow[train_edge_all[i][1]])
        feature_train_0.append(feature_shadow[train_edge_all[i][0]])
        feature_train_1.append(feature_shadow[train_edge_all[i][1]])
        if i >num_train_shadow:
        ...
        for j in range(len(sample_list)):
            for k in range(j):
                if adj[sample_list[j]][sample_list[k]]==1:
                    ...
                    for l in range(feature.shape[1]):
                        feature_test_0.append(feature[sample_list[k]][l])
                        feature_test_1.append(feature[sample_list[j]][l])
    ...
    test_embed_false_0=[]
    test_embed_false_1=[]
    test_feature_false_0 = []
    test_feature_false_1 = []
    test_label_false=[]
    for i in range (len(test_edge_false)):
        ...
        test_feature_false_0.append(feature[test_edge_false[i][0]])
        test_feature_false_1.append(feature[test_edge_false[i][1]])
    ...
    test_feature_false_0 = np.array(test_feature_false_0)
    test_feature_false_1 = np.array(test_feature_false_1)
    ...
    feature_train_1 = np.array(feature_train_1)
    feature_train_0 = np.array(feature_train_0)
    feature_test_0 = np.array(feature_test_0)
    feature_test_1 = np.array(feature_test_1)
    ...
    feature_test_0 = feature_test_0.reshape(int(len(feature_test_0) / (feature.shape[1])), int(feature.shape[1]))
    feature_test_1 = feature_test_1.reshape(int(len(feature_test_1) / (feature.shape[1])), int(feature.shape[1]))
    ...
    feature_test_0 = np.vstack((feature_test_0, test_feature_false_0))
    feature_test_1 = np.vstack((feature_test_1, test_feature_false_1))
    ...
    for i in range (embed_train_0.shape[0]):
        ...
        feature_train = np.array([row(feature_train_0[i], feature_train_1[i]) for row in similarity_list])
        train_vec = get_metrics(embed_train_0[i], embed_train_1[i], "entropy", operator_func)
        embed_train = np.concatenate(( embed_train, train_vec,feature_train))
    ...
    for i in range (embed_test_1.shape[0]):
        ...
        feature_test = np.array([row(feature_test_0[i], feature_test_1[i]) for row in similarity_list])
        test_vec = get_metrics(embed_test_0[i], embed_test_1[i], "entropy", operator_func)
        embed_test = np.concatenate((embed_test, test_vec,feature_test))
...
def get_roc_score(pred,ture):
    ...
    ap = average_precision_score(ture_list,pred_list)
    return roc,ap
...
def metric_3(ori_adj, inference_adj, idx):
    ...
    AP=average_precision_score(real_all,pred_all)

    print("Inference attack AUC: %f,Inference attack AP: %f" % (AUC,AP))
...
parser.add_argument('--dataset', type=str, default='cora',
                    choices=['cora', 'cora_ml', 'citeseer', 'polblogs', 'pubmed', 'AIDS', 'usair', 'brazil','PROTEINS_full'], help='dataset')
...
# np.save('result/result_MI_view_%s'%args.dataset,inference_adj_2.cpu().detach().numpy())
# test_load=np.load('result/result_MI_view_%s.npy'%args.dataset)
...
# data_shadow = Dataset(root='', name=args.shadowdataset, setting='GCN')
# adj_shadow, features_shadow, labels_shadow, init_adj_shadow = data_shadow.adj, data_shadow.features, data_shadow.labels, data_shadow.init_adj
#
# idx_train_shadow, idx_val_shadow, idx_test_shadow = data_shadow.idx_train, data_shadow.idx_val, data_shadow.idx_test
...
feature_adj_shadow = dot_product_decode(features_shadow)
#preprocess_adj = preprocess_Adj(adj, feature_adj)
init_adj = torch.FloatTensor(init_adj.todense())

init_adj_shadow = torch.FloatTensor(init_adj_shadow.todense())
...
adj_vic_noraml_shadow=normalize_adj_tensor(adj_vic_shadow)
embed_shadow=victim_model_shadow(features_vic_shadow,adj_vic_noraml_shadow)
...
def main():
    ...
    if embed_shadow.cpu().detach().shape[1] > embed.cpu().detach().shape[1]:
        zero_=np.zeros(shape=(embed.shape[0],(embed_shadow.shape[1]-embed.shape[1])))
        embed_d=np.array(embed.cpu().detach())
        embed_d=np.hstack((embed_d,zero_))
        embed_d = torch.from_numpy(embed_d)
        embed_shadow_d=embed_shadow.cpu().detach()


    else:
        zero_ = np.zeros(shape=(embed_shadow.shape[0], (embed.shape[1] - embed_shadow.shape[1])))
        embed_shadow_d = np.array(embed_shadow.cpu().detach())
        embed_shadow_d = np.hstack((embed_shadow_d, zero_))
        embed_shadow_d = torch.from_numpy(embed_shadow_d)
        embed_d = embed.cpu().detach()



    sample_adj,sample_features,sample_list=sample(adj_shadow,embed_shadow_d.cpu().detach(),96)
    sample_adj=sample_adj.type(torch.float32)
    sample_features=sample_features.type(torch.float32)

    sample_adj_or, _, _ = sample(adj, embed.cpu().detach(), 96)
    ...
    embed_train,embed_test,label_train,label_test,addr_train,addr_test=find_label_embed(adj,embed.cpu().detach().numpy(),features.cpu().detach().numpy(),adj_shadow,embed_shadow.cpu().detach().numpy(),features_shadow.cpu().detach().numpy(),96)
    ...
    for i in range(200):
        ...
        loss.backward()
        optimizer.step()
        roc,ap=get_roc_score(preds,label_train.to(device))
        print("roc:", roc)
        print("ap:", ap)
    ...
    roc,ap = get_roc_score(infer, label_test.to(device))
    print("infer_roc:", roc)
    print("ap:", ap)
    ...
    adj_infer = pingjie_2(inference_adj, addr_test, infer)
    # metric(adj.numpy(),adj_infer.detach().cpu().numpy(),idx_attack)




    sample_adj_test, sample_features_test ,sample_list= sample_infer(adj, embed_d,adj_infer.cpu().detach(), 96)
    sample_adj_test=sample_adj_test.type(torch.float32)
