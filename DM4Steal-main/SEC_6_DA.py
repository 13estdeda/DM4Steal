...
def main():
    ...
    print("1")
    sample_adj, sample_features, sample_list = sample_with_shadow(adj, embed.cpu().detach(), adj_shadow,
                                                                  embed_shadow.cpu().detach(), 96)
    sample_adj = sample_adj.type(torch.float32)
    sample_features = sample_features.type(torch.float32)
    ...
    sample_adj_test, sample_features_test,sample_list = sample_infer(adj, embed.cpu().detach(),inference_adj,embed_shadow.cpu().detach(), args.batch,args.maxsize,args.minsize)
    sample_adj_test=sample_adj_test.type(torch.float32)
