...
from util.arg_helper import edict2dict, parse_arguments, get_config, process_config, set_seed_and_logger
...
parser.add_argument('--shadowdataset', type=str, default='cora',
                    choices=['cora', 'cora_ml', 'citeseer', 'polblogs', 'pubmed', 'AIDS', 'usair', 'brazil'], help='dataset')
...
parser.add_argument('--minsize',type=int,default=1)
...
def main():
    ...
    print("1")
    sample_adj, sample_features, sample_list = sample_with_shadow(adj, features.cpu().detach(), adj_shadow,
                                                                  features_shadow.cpu().detach(), 96)
    sample_adj = sample_adj.type(torch.float32)
    sample_features = sample_features.type(torch.float32)
    ...
    sample_adj_test, sample_features_test,sample_list = sample_infer(adj, features.cpu().detach(),inference_adj,features_shadow.cpu().detach(), args.batch,args.maxsize,args.minsize)
    sample_adj_test=sample_adj_test.type(torch.float32)
