...
def train_main(config, args,train_adj,train_x,test_adj,test_x):
    ...
    sample_b,sample_time=fit(model, optimizer, mc_sampler, train_adj,train_x,test_adj,test_x,
...
parser.add_argument('--defense', type=str, default="no_defense", choices=['DP_gauss', 'DP_laplace', 'noise', 'droup_out'],help="defense methods")
...
def main():
    ...
    time_graphMI=time.time()
    model.attack(features, init_adj, labels, idx_attack, num_edges, epochs=args.epochs)
    time_graphMI=time.time()-time_graphMI
    #
    inference_adj = model.modified_adj.cpu()
    # np.save('result/result_MI_{}_{}'.format(args.dataset,args.defense),inference_adj.numpy())
    # np.save('result/result_MI_index_{}_{}'.format(args.dataset,args.defense),idx_attack)
    ...
    sample_adj_test, sample_features_test = sample_infer(adj, features.cpu().detach(),inference_adj, 96)
    sample_adj_test=sample_adj_test.type(torch.float32)
    ...
    sample_adj_b,time_sample=train_main(config_dict,args_2,sample_adj,sample_features,sample_adj_test,sample_features_test)
    ...
    # metric(adj.numpy(), inference_adj.numpy(), idx_attack)
    print('time_lsa:',time_graphMI,'time_sample',time_sample)
    print('shape_lsa',inference_adj.shape,'shape_sample',sample_adj_b.shape)
