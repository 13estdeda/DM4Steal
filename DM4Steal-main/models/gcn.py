...
class embedding_GCN(nn.Module):
    def __init__(self, nfeat, nhid, defense='none',with_bias=True, device=None):
    ...
    def forward(self, x, adj):
        ...
        if self.defense == 'DP_gauss':
            noise = DP_noise(1, x.shape, mode='gauss').to(self.device)
            x = x + noise
        elif self.defense == 'DP_laplace':
            DP_noise(1, x.shape, mode='laplace').to(self.device)
            x = x + DP_noise
        elif self.defense == 'noise':
            x = x + torch.randn(x.shape).to(self.device)
        elif self.defense == 'droup_out':
            x = F.dropout(x, p=0.8)
        else:
            print('no defense')
...
class GCN(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4, defense='none', with_relu=True, with_bias=True, device=None):
    ...
    def fit(self, features, adj, labels, idx_train, idx_val=None, train_iters=100, initialize=True, verbose=True, normalize=True, patience=500, **kwargs):
        ...
        # flag=utils.is_sparse_tensor(adj)
