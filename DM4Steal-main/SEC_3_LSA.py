...
def find_label_embed(adj,embed,batch,feature):
    # adj=adj.cpu().detach()
    ...
    for i in range (len(addr_train)):
        ...
        feature_train_0.append(feature[train_edge_all[i][0]])
        feature_train_1.append(feature[train_edge_all[i][1]])
        if i >num_train:
            ...
            for k in range(j):
                if adj[sample_list[j]][sample_list[k]]==1:
                    ...
                    for l in range(feature.shape[1]):
                        ...
                        feature_test_1.append(feature[sample_list][k][l])
    ...
    for i in range (embed_train_0.shape[0]):
        ...
        embed_train = np.concatenate((fea_train, embed_train, train_vec,feature_train))
    ...
    for i in range (embed_test_1.shape[0]):
        ...
        embed_test = np.concatenate((fea_test,embed_test, test_vec,feature_test))
...
def sample_with_shadow(adj,features,adj_shadow,features_shadow,batch):
    if features.shape[1]>features_shadow.shape[1]:
        zero=np.zeros((features_shadow.shape[0],int(features.shape[1]-features_shadow.shape[1])))
        features_shadow = np.array(features_shadow)
        features_shadow=np.hstack((features_shadow,zero))
        features_shadow=torch.from_numpy(features_shadow)
    else:
        zero=np.zeros((features.shape[0],int(features_shadow.shape[1]-features.shape[1])))
        features = np.array(features)
        features=np.hstack((features,zero))
    ...
    for i in range(adj.shape[0]):
        ...
        if len(list1)>batch:
            print("find subgraph")
            break
    init_adj=np.zeros((batch+2,16,16))
    init_feature=np.zeros((batch+2,16,features.shape[1]))
    for i in range(batch):
        sample_list = []
        sample_batch=adj[list1[i]]
        init_feature[i][0]=features[list1[i]]
    ...
    for i in range(adj_shadow.shape[0]):
        if sum(adj_shadow[i]) > 1 and sum(adj_shadow[i]) < 16:
            # z=sum(adj_shadow[i])
            list1.append(i)
        else:
            list2.append(i)
        if len(list1) > batch+2:
            print("find subgraph")
            break
    for i in range(batch,batch+2):
        sample_list = []
        sample_batch = adj_shadow[list1[i+1]]
        # sample_batch = np.array(sample_batch)
        init_feature[i][0] = features_shadow[list1[i+1]]
        t = time.time()
        for j in range(sample_batch.shape[0]):
            if sample_batch[j] == 1:
                sample_list.append(j)
        ...
        for j in range(len(sample_list)):
            init_adj[i][0][j + 1] = init_adj[i][j + 1][0] = 1
            for k in range(len(sample_list)):
                if adj_shadow[sample_list[j]][sample_list[k]] == 1:
                    init_adj[i][j + 1][k + 1] = init_adj[i][k + 1][j + 1] = 1
                    init_feature[i][j + 1] = features_shadow[sample_list[j]]
        print("shijian2:", time.time() - t)
...
def main():
    ...
    sample_adj,sample_features,sample_list=sample_with_shadow(adj,features.cpu().detach(),adj_shadow,features_shadow.cpu().detach(),96)
    ...
    embed_train,embed_test,label_train,label_test,addr_train,addr_test=find_label_embed(adj,embed.cpu().detach().numpy(),96,features.cpu().detach().numpy())
    ...
    sample_adj_test, sample_features_test = sample_infer(adj, features.cpu().detach(),adj_infer.cpu().detach(),features_shadow.cpu().detach(), 96)
    sample_adj_test=sample_adj_test.type(torch.float32)
