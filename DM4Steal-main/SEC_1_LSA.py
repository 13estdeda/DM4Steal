...
def pingjie_2(adj,addr,infer):
    ...
    for i in range(addr.shape[0]):
        adj[addr[i][0]][addr[i][1]]=infer[i][0]

    return adj


def find_link_unlink(adj):
...
def pingjie(adj,infer_adj,sample_adj,batch):
    ...
    for i in range(adj.shape[0]):
        if sum(adj[i]) > 0 and sum(adj[i]) < 16:
            list1.append(i)
        else:
            list2.append(i)
    for i in range(batch):
        sample_list = []
        sample_batch = adj[list1[i+32]]
        for j in range(sample_batch.shape[0]):
            if sample_batch[j] == 1:
                sample_list.append(j)
        for j in range(len(sample_list)):
            infer_adj[sample_list[j]][list1[i+32]] = infer_adj[list1[i+32]][sample_list[j]] = sample_adj[i][j+1][0]
...
#         for j in range(len(sample_list)):
#             for k in range(j):
#                 label_train.append(adj[sample_list[j]][sample_list[k]])
#                 label_train.append(1-adj[sample_list[j]][sample_list[k]])
#                 addr_train.append(sample_list[k])
#                 addr_train.append(sample_list[j])
#                 for l in range(embed.shape[1]):
#                     embed_train_0.append(embed[sample_list[k]][l])
#                     embed_train_1.append(embed[sample_list[j]][l])
#
#
#     for i in range(batch):
#         sample_list=[]
#         sample_list.append(list1[i+32])
#         sample_batch = adj[list1[i+32]]
#         for j in range(sample_batch.shape[0]):
#             if sample_batch[j]==1:
#                 sample_list.append(j)
...
#
#     addr_train=np.array(addr_train)
#     addr_test=np.array(addr_test)
#     label_train=np.array(label_train)
#     label_test=np.array(label_test)
#     label_train=label_train.reshape(int(len(label_train)/2),2)
#     label_test = label_test.reshape(int(len(label_test) / 2), 2)
#     embed_train_0=embed_train_0.reshape(int(len(embed_train_0)/(embed.shape[1])),int(embed.shape[1]))
#     embed_train_1 = embed_train_1.reshape(int(len(embed_train_1) / (embed.shape[1])), int(embed.shape[1]))
...
#     addr_train=addr_train.reshape(int(len(addr_train) / 2), 2)
...
def find_label_embed(adj,embed,batch):
    ...
    embed_test_1 = embed_test_1.reshape(int(len(embed_test_1) / (embed.shape[1])), int(embed.shape[1]))
    addr_test = addr_test.reshape(int(len(addr_test) / 2), 2)
    print("embed_test_0.shape:",embed_test_0.shape)
    print("embed_test_0.shape:",test_embed_false_0[0])
...
# attack_model = steal_link_6(40,2)
...
def main():
    ...
    sample_adj,sample_features,sample_list=sample(adj,embed.cpu().detach(),96)
    ...
    # label_train_link, label_train_unlink, addr_train_link, addr_train_unlink=find_link_unlink(adj)
    ...
    embed_train,embed_test,label_train,label_test,addr_train,addr_test=find_label_embed(adj,embed.cpu().detach().numpy(),96)
    ...
    sample_adj_test, sample_features_test = sample_infer(adj, embed.cpu().detach(),adj_infer.cpu().detach(), 96)
    sample_adj_test=sample_adj_test.type(torch.float32)
