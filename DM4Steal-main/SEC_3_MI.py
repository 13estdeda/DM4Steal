...
parser.add_argument('--shadowdataset', type=str, default='ENZYMES',
                    choices=['cora', 'cora_ml', 'citeseer', 'polblogs', 'pubmed', 'AIDS', 'COX2', 'ENZYMES','PROTEINS_full'], help='dataset')
...
def main():
    ...
    # inference_adj = np.load('result/result_GAT_MI_%s.npy' % args.dataset)
    # inference_adj=np.load('result/result_MI_{}_{}.npy'.format(args.dataset,args.defense))
    # idx_attack = np.load('result/result_GAT_MI_index_%s.npy' % args.dataset)
    ...
    sample_adj_test, sample_features_test,sample_list = sample_infer(adj, features.cpu().detach(),inference_adj, features_shadow.cpu().detach(),96)
    sample_adj_test=sample_adj_test.type(torch.float32)
    ...
    # metric_3(sample_adj.cpu().numpy(),sample_adj_b.cpu().numpy(), sample_list)
