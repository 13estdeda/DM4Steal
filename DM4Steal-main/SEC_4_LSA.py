...
def find_label_embed(adj,embed,adj_shadow,embed_shadow,batch):
    # adj=adj.cpu().detach()
    ...
    for i in range (embed_train_0.shape[0]):
        ...
        embed_train = np.concatenate(( embed_train, train_vec))
    ...
    for i in range (embed_test_1.shape[0]):
        ...
        embed_test = np.concatenate((embed_test, test_vec))
...
parser.add_argument('--shadowdataset', type=str, default='COX2',
...
def main():
    ...
    embed_train,embed_test,label_train,label_test,addr_train,addr_test=find_label_embed(adj,embed.cpu().detach().numpy(),adj_shadow,embed_shadow.cpu().detach().numpy(),96)
